...
x = 0.2
wing_area_total = 60
chord_root = m.sqrt(wing_area_total/aspect_ratio)*(0.5/(0.35+0.15*x))
#print(chord_root)
chord_tip = 0.4*chord_root
wing_area_outside_fuselage = wing_area_total-x*m.sqrt(wing_area_total)*m.sqrt(aspect_ratio)*chord_root
...
for n in range(1, number_of_wing_segments):
    ...
    local_torsion = torsion_force_previous + wing_box_chord_width*0.17*(local_load+local_load_previous)/2*(spanwise_locations[-n-1]-spanwise_locations[-n])
    ...
    thickness_required_torsion = 0.0001       
    running = True
    while running == True:
        torsional_stress = -local_torsion/(2*thickness_required_torsion*wing_box_chord_height*wing_box_chord_width)
    ...
    torque_distribution.append(local_torsion)
    torsion_force_previous = local_torsion
    ...
    torsion_plate_thickness_list.append(thickness_required_torsion)
...
bending_moment_middle_section = -x/2/2*m.sqrt(wing_area_total*aspect_ratio)*lift_half_plane_middle_section+x/2*m.sqrt(wing_area_total*aspect_ratio)*fuselage_weight*9.81+bending_moment_distribution[0]+local_shear_distribution[0]*-x/2*m.sqrt(wing_area*aspect_ratio)
...
print((mass_middle_section+mass_wing_box*2)*2)