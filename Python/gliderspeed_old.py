...
reel_factor_array = np.arange(0.3, 1, 0.1) #m/s
...
def apparent_wind_speed_values(windspeed, reelspeed, Lambda, polar_angle, azimuth_angle, hi):
    weather_factor = 11.4688/11.4688
    kite_speed_tangential = Lambda * windspeed
    wind_component = np.matrix([[np.sin(polar_angle) * np.cos(azimuth_angle)], [np.cos(polar_angle) * np.cos(azimuth_angle)], [-np.sin(azimuth_angle)]]) * (windspeed*weather_factor)
...
print("V_diff = ", max_apparent_wind_speed_magnitude - 87.72224698611024)
print("P_diff = ", corresponding_generator_power - 635042.7265856236)
